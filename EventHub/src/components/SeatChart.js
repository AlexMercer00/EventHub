...
const SeatChart = ({ occasion, EventHub, provider, setToggle }) => {
  const [seatsTaken, setSeatsTaken] = useState(false)
  const [hasSold, setHasSold] = useState(false)

  const getSeatsTaken = async () => {
    const seatsTaken = await EventHub.getSeatsTaken(occasion.id)
  ...
  const buyHandler = async (_seat) => {
    ...
    const transaction = await EventHub.connect(signer).mint(occasion.id, _seat, { value: occasion.cost })
